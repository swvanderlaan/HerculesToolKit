...
echo "Today's:" `date`
echo ""
echo "Version: LOOKUP_DATA.v2.0.20160217"
echo ""
echo "Last update: February 17th, 2016"
echo "Written by: Sander W. van der Laan (s.w.vanderlaan-2@umcutrecht.nl)"
echo ""
echo " Lookup variants in (imputed) ExomeChip or GWAS data, or lookup genes in VEGAS "
echo " results. "
echo ""
echo " The input files are assumed to be so-called *.DAT.GZ files, i.e. gzipped dat-files, "
	...
	if [[ $TYPE = "VARIANT" ]]; then 
		...
		while read PHENOTYPES; do 
			for TRAIT in ${PHENOTYPES}; do
				...
				zcat ${INPUTDIR}/${BASEFILENAME}.${TRAIT}.dat.gz | head -1 > ${OUTPUTDIR}/${TRAIT}.${PROJECT}.${TYPE}.txt
				echo "Un-gzipping data..."
				gzip -dv ${INPUTDIR}/${BASEFILENAME}.${TRAIT}.dat.gz 
				echo "Done. Processing results for '${TRAIT}'."
				perl $SOFTWARE/overlap.pl ${LOOKUP} 1 ${INPUTDIR}/${BASEFILENAME}.${TRAIT}.dat 2 >> ${OUTPUTDIR}/${TRAIT}.${PROJECT}.${TYPE}.txt
				echo "Done. Gzipping results for '${TRAIT}'."
				gzip -v ${INPUTDIR}/${BASEFILENAME}.${TRAIT}.dat
